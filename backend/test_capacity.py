...
def test_capacity():
    ...
    logger.info("Creating Trainer A...")
    ts = int(time.time())
    ...
    logger.info("Creating Trainer B...")
    ...
    logger.info(f"Book 1: {resp.status_code} (Expected 200)")
    
    # Book 2
    resp = requests.post(f"{BASE_URL}/appointments/", json={
        "trainer_id": t1['id'], "client_name": "C2", "client_email": f"c2_{ts}@test.com", "start_time": TEST_TIME
    })
    logger.info(f"Book 2: {resp.status_code} (Expected 200)")
    ...
    logger.info(f"Book 3: {resp.status_code} (Expected 400 - Trainer Full)")
    if resp.status_code == 400:
        logger.info("PASS: Trainer capacity limit enforced.")
    else:
        logger.error("FAIL: Trainer capacity check failed.")
    ...
    logger.info(f"Book T2-1: {resp.status_code} (Expected 200)")
    
    # Book 2 for T2 (Total = 4 -> Should Fail)
    resp = requests.post(f"{BASE_URL}/appointments/", json={
        "trainer_id": t2['id'], "client_name": "C5", "client_email": f"c5_{ts}@test.com", "start_time": TEST_TIME
    })
    logger.info(f"Book T2-2: {resp.status_code} (Expected 400 - Gym Full)")
    if resp.status_code == 400 and "Gym capacity" in resp.text:
        logger.info("PASS: Gym global capacity limit enforced.")
    else:
        logger.error(f"FAIL: Gym capacity check failed. Status: {resp.status_code}, Body: {resp.text}")
